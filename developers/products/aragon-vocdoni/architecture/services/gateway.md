...
![](https://docs.vocdoni.io/gateway-components.png)

### Discovery mechanism <a href="#discovery-mechanism" id="discovery-mechanism"></a>
...
## API definition <a href="#api-definition" id="api-definition"></a>
...
The API methods below follow the [JSON API](https://docs.vocdoni.io/architecture/protocol/json-api) specifications.

## Info API <a href="#info-api" id="info-api"></a>

### Get Gateway Info <a href="#get-gateway-info" id="get-gateway-info"></a>
...
## Census API <a href="#census-api" id="census-api"></a>

The Census API methods can be found on the [Census Service section](https://docs.vocdoni.io/architecture/services/census-service.html#json-api).

## Vote API <a href="#vote-api" id="vote-api"></a>

### Submit Raw Vochain Transaction <a href="#submit-raw-vochain-transaction" id="submit-raw-vochain-transaction"></a>

Send a [Vote Envelope](https://docs.vocdoni.io/architecture/smart-contracts/process.html#vote-envelope) to the mempool of the [Vochain](https://docs.vocdoni.io/architecture/services/vochain).
...
### Submit Envelope <a href="#submit-envelope" id="submit-envelope"></a>
...
Check the status of an already submitted [Vote Envelope](https://docs.vocdoni.io/architecture/smart-contracts/process.html#vote-envelope). The envelope is identified by the voter's nullifier.
...
### Get Envelope Height <a href="#get-envelope-height" id="get-envelope-height"></a>
...
### Get Block Height <a href="#get-block-height" id="get-block-height"></a>

Get the current block number on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain).
...
### Get Process Keys <a href="#get-process-keys" id="get-process-keys"></a>
...
[Vote Package](https://docs.vocdoni.io/architecture/smart-contracts/process.html#vote-package) encryption and decryption keys are expected to follow the order of their indexes. Smaller indexes are used first and it's important to note that indexes might not be consecutive.
...
### Get Block Status <a href="#get-block-status" id="get-block-status"></a>
...
### Get pre-register voter weight <a href="#get-pre-register-voter-weight" id="get-pre-register-voter-weight"></a>
...
## Results API <a href="#results-api" id="results-api"></a>

### Get Process List <a href="#get-process-list" id="get-process-list"></a>

Get a list of processes for a specific entity or namespace on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain). There is a hardcoded maximum size of 64 per page.
...
* `sourceNetworkId` can be used for querying only a specific network origin (where the process has been created). Current possible values are: UNKNOWN, ETH\_MAINNET, ETH\_RINKEBY, ETH\_GOERLI, POA\_XDAI, POA\_SOKOL, POLYGON, BSC, ETH\_MAINNET\_SIGNALING, ETH\_RINKEBY\_SIGNALING An empty string is also allowed (means no filter).
...
### Get Process Info <a href="#get-process-info" id="get-process-info"></a>
...
### Get Process Summary <a href="#get-process-summary" id="get-process-summary"></a>
...
### Get Process Count <a href="#get-process-count" id="get-process-count"></a>

Returns the number of processes registered on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain).
...
### Get Process Results <a href="#get-process-results" id="get-process-results"></a>
...
The results of an election are represented in [the following format](https://docs.vocdoni.io/architecture/smart-contracts/process.html#results).
...
### Get Results Weight <a href="#get-results-weight" id="get-results-weight"></a>
...
### Get Entity List <a href="#get-entity-list" id="get-entity-list"></a>

Get a list of entities that created at least 1 process on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain).
...
### Get Entity Count <a href="#get-entity-count" id="get-entity-count"></a>

Returns the number of entities registered on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain) (those that have created at least one process).
...
### Get Envelope <a href="#get-envelope" id="get-envelope"></a>

Get the content of an existing [Vote Envelope](https://docs.vocdoni.io/architecture/smart-contracts/process.html#vote-envelope). The envelope is identified by the nullifier. `height` and `txIndex` refer to the block height and the index on that block, respectively, of the transaction containing this vote envelope. These fields can be used with [Get Tx](https://docs.vocdoni.io/architecture/services/gateway.html#get-tx) to fetch this transaction.
...
## Indexer API <a href="#indexer-api" id="indexer-api"></a>

### Get Stats <a href="#get-stats" id="get-stats"></a>

Get general information & statistics about the current [Vochain](https://docs.vocdoni.io/architecture/services/vochain) status.
...
### Get Envelope List <a href="#get-envelope-list" id="get-envelope-list"></a>

Get a list of nullifiers for votes registered on a given process ID (at most, 64 per request).

The `fromId` field works the same as in [Get Process List](https://docs.vocdoni.io/architecture/services/gateway.html#get-process-list).
...
Get the metadata for a single block on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain) by its height. `num_txs` contains the number of transactions contained in this block. In order to access these transactions (the contents of the block), use [Get Tx List for Block](https://docs.vocdoni.io/architecture/services/gateway.html#get-tx-list-for-block).
...
### Get Block By Hash <a href="#get-block-by-hash" id="get-block-by-hash"></a>
...
### Get Block List <a href="#get-block-list" id="get-block-list"></a>

Get a list of confirmed blocks on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain) (at most, 64 per request).
...
### Get Tx <a href="#get-tx" id="get-tx"></a>

Get a single tx from the [Vochain](https://docs.vocdoni.io/architecture/services/vochain). `height` is the block containing a transaction and `txIndex` in the transaction's index on that block.
...
### Get Tx By Height <a href="#get-tx-by-height" id="get-tx-by-height"></a>

Get a single tx from the [Vochain](https://docs.vocdoni.io/architecture/services/vochain). `height` is that transaction's height on the vochain as a whole, and each height value references a unique transaction. Transaction height corresponds to the order in which that transaction was mined.
...
### Get Transaction By Hash <a href="#get-transaction-by-hash" id="get-transaction-by-hash"></a>
...
### Get Validator List <a href="#get-validator-list" id="get-validator-list"></a>

Get the list of all addresses currently validating blocks on the [Vochain](https://docs.vocdoni.io/architecture/services/vochain).
...
### Get Tx List For Block <a href="#get-tx-list-for-block" id="get-tx-list-for-block"></a>
...
## File API <a href="#file-api" id="file-api"></a>

### Fetch File <a href="#fetch-file" id="fetch-file"></a>
...
* [Voting process retrieval](https://docs.vocdoni.io/architecture/component-interaction.html#voting)
* [Vote scrutiny](https://docs.vocdoni.io/architecture/component-interaction.html#after-voting)

**Related:**

* [Content URI](https://docs.vocdoni.io/architecture/protocol/data-origins.html#content-uri)

### Add File <a href="#add-file" id="add-file"></a>
...
These methods require authentication, following the [JSON API rules](https://docs.vocdoni.io/architecture/protocol/json-api.html#authentication).
...
* [Voting process creation](https://docs.vocdoni.io/architecture/component-interaction.html#voting-process-creation)

**Related:**

* [Content URI](https://docs.vocdoni.io/architecture/protocol/data-origins.html#content-uri)

### List pinned files <a href="#list-pinned-files" id="list-pinned-files"></a>
...
### Pin a file <a href="#pin-a-file" id="pin-a-file"></a>
...
### Unpin a file <a href="#unpin-a-file" id="unpin-a-file"></a>
...
## Using the API (for beginners) <a href="#using-the-api-for-beginners" id="using-the-api-for-beginners"></a>

If you don't have experience using a [JSON API](https://docs.vocdoni.io/architecture/protocol/json-api), let's run through an example. Here we'll be using [curl](https://linux.die.net/man/1/curl) to make requests to https://gw1.vocdoni.net, one of the gateways hosted by Vocdoni.
...
now we know that the gateway hosted at gw1.vocdoni.net enables the file, [vote](https://docs.vocdoni.io/architecture/services/gateway.html#vote-api), [results](https://docs.vocdoni.io/architecture/services/gateway.html#results-api), and [indexer](https://docs.vocdoni.io/architecture/services/gateway.html#indexer-api) apis.
